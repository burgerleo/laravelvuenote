...
new Vue({
  ...
  methods: {
    ...
    createnote: function() {
      ...
      this.$http.post('http://localhost/laravelvuenote/public/vuenotes',input).then((response) => {
    ...
    deletenote: function(note) {
      this.$http.delete('http://localhost/laravelvuenote/public/vuenotes/'+note.id).then((response) => {
        this.changePage(this.pagination.current_page);
        toastr.error('刪除成功', '成功訊息', {timeOut: 5000});
      });
    },
    ...
    updatenote: function(id) {
      ...
      this.$http.put('http://localhost/laravelvuenote/public/vuenotes/'+id,input).then((response) => {
        this.changePage(this.pagination.current_page);
        this.newnote = {'title':'','content':'','id':''};
        $("#edit-note").modal('hide');
        toastr.info('更新成功', '成功訊息', {timeOut: 5000});
      }, (response) => {
        this.formErrors = response.data;
      });
    },
